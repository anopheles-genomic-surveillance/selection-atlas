...
def get_h12_calibration_yamls(wildcards):
    ...
    paths = f"{build_dir}/h12-calibration/" + df['cohort_id'] + ".yaml"
    return paths
...
def get_selection_atlas_outputs(wildcards):
    ...
    h12_cal_paths = f"{build_dir}/h12-calibration/" + df['cohort_id'] + ".yaml"
    h12_cal_notebook_paths = f"{build_dir}/notebooks/h12-calibration-" + df['cohort_id'] + ".ipynb"
    h12_gwss_paths = f"{build_dir}/notebooks/h12-gwss-" + df['cohort_id'] + ".ipynb"
    h12_signal_paths = expand("{build_dir}/h12-signal-detection/{cohort}_{contig}.csv", cohort=df['cohort_id'], contig=chromosomes, build_dir=build_dir)

    # define paths to output files
    g123_cal_paths = f"{build_dir}/g123-calibration/" + df['cohort_id'] + ".yaml"
    g123_cal_notebook_paths = f"{build_dir}/notebooks/g123-calibration-" + df['cohort_id'] + ".ipynb"
    g123_gwss_paths = f"{build_dir}/notebooks/g123-gwss-" + df['cohort_id'] + ".ipynb"
    
    ihs_gwss_paths = f"{build_dir}/notebooks/ihs-gwss-" + df['cohort_id'] + ".ipynb"
